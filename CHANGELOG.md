...
- 新增 `IMPROVEMENTS.md` 文档，详细记录汉化改进过程和效果
- 新增 190+ 个 spinner 动词，总数从 190 个增加到 380+ 个，涵盖：
  - 修仙系列（炼丹中、修仙中、渡劫中等 20+ 个）
  - 打工系列（摸鱼中、划水中、躺平中、内卷中等）
  - 开发运维系列（重构中、优化中、压测中、灰度中等 30+ 个）
  - 版本控制系列（拉取中、推送中、合并中、冲突中等）
  - 网络通信系列（连接中、断开中、重连中、超时中等）
  - 数据处理系列（编码中、解码中、压缩中、解压中等）
  - 并发控制系列（并发中、并行中、串行中、同步中等）
  - 服务治理系列（健康检查中、服务发现中、注册中、心跳中等）

### 改进

- **大幅增强 session-start 中文上下文**（从 ~200 字扩展到 ~2000 字）：
  - 新增"语言和风格"部分，明确中文表达习惯
  - 新增"技术术语处理"部分，详细列出保留英文和可翻译的术语
  - 扩展"错误信息翻译参考"从 10 条到 60+ 条，覆盖文件系统、网络、Git、HTTP、构建等各类错误
  - 新增"中文表达习惯"部分，指导自然的中文表达
  - 新增"代码相关表达"部分，规范代码和技术内容的呈现方式

- **扩展 notification hook 翻译覆盖**（从 6 种到 40+ 种）：
  - 新增网络错误翻译（连接被拒绝、连接超时、DNS 解析失败等）
  - 新增认证和权限错误翻译（认证失败、权限被拒绝、访问被拒绝等）
  - 新增文件操作错误翻译（文件未找到、目录未找到、磁盘空间不足等）
  - 新增 Git 操作错误翻译（合并冲突、分支未找到、远程仓库未找到等）
  - 新增包管理错误翻译（包未找到、依赖错误、安装失败等）
  - 新增构建和编译错误翻译（构建失败、编译错误、语法错误等）
  - 新增服务器错误翻译（500、502、503、504 等）

- **优化 output-styles 中文风格**（从 ~150 字扩展到 ~1500 字）：
...
- **中文上下文指导**：内容扩展 10 倍，提供更详细的中文表达指导
- **错误信息翻译**：覆盖率提升 6 倍，涵盖开发过程中的各类常见错误
- **通知翻译覆盖**：覆盖率提升 7 倍，用户看到的系统通知有更高的中文覆盖率
- **输出风格指导**：内容扩展 10 倍，Claude 的中文回复更加自然流畅
- **Spinner 动词**：数量翻倍，涵盖开发、运维、网络等各个领域

### 用户体验改善

- Claude 的中文回复更加自然、流畅，符合中文用户的阅读习惯
- 错误信息有更高的中文覆盖率，更容易理解
- 系统通知更容易理解，减少查阅文档的需求
- 等待过程更有趣味性，动词涵盖开发全流程
- 技术术语使用更规范，避免过度翻译或翻译不足
